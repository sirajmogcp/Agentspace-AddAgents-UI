...
async function submitAddAgentForm(event) {
    ...
    try {
        const response = await fetch('/api/as-agents/create-agent', {
            ...
            body: JSON.stringify({
                ...
                adk_deployment_id: formData.get('adk_deployment_id'),
                auth_id: formData.get('auth_id'),
                icon_uri: formData.get('icon_uri')
            })
        });
        ...
        // Reload the agents list if we're in the show agents modal
        const showAgentsAppId = document.getElementById('showAgentsAppIdDisplay').textContent;
        if (showAgentsAppId) {
            loadAgents(showAgentsAppId);
        }
