...
@app.route('/api/agentspace-apps/<int:app_db_id>/agents', methods=['POST'])
def add_agent_to_app(app_db_id):
    agentspace_app = find_agentspace_app(app_db_id)
    
    if not request.json:
        return jsonify({"error": "Missing request body"}), 400
    
    required_fields = ['display_name', 'description', 'tool_description', 'adk_deployment_id']
    ...
    result = create_agent(
        ...
        app_id=agentspace_app['appId'],
        display_name=request.json['display_name'],
        description=request.json['description'],
        tool_description=request.json['tool_description'],
        adk_deployment_id=request.json['adk_deployment_id'],
        auth_id=request.json.get('auth_id'),
        icon_uri=request.json.get('icon_uri')
    )
    
    if 'error' in result:
        return jsonify(result), 400
    
    return jsonify({"message": "Agent added successfully", "agent": result}), 201

@app.route('/api/agentspace-apps/<int:app_db_id>/agents', methods=['GET'])
def get_app_agents(app_db_id):
    try:
        agentspace_app = find_agentspace_app(app_db_id)
        
        # Get project_id and app_id from the agentspace_app
        project_id = agentspace_app.get('project_id')
        app_id = agentspace_app.get('appId')
        
        if not project_id or not app_id:
            return jsonify({"error": "Project ID or App ID not found in agentspace app"}), 400
        ...
        print(f"Fetching agents for project_id: {gcp_project_id}, app_id: {app_id}")  # Debug print
        
        # Use list_agents from as_agent_registry_service
        result = list_agents(
        ...
        print(f"Error in get_app_agents: {str(e)}")  # Debug print
        return jsonify({"error": str(e)}), 500

@app.route('/api/agentspace-apps/<int:app_db_id>/agents/<int:agent_id>', methods=['DELETE'])  # Changed from categories to agents
def delete_agent_from_app(app_db_id, agent_id):  # Changed from delete_category_from_app
    agentspace_app = find_agentspace_app(app_db_id)
    agent_to_delete = find_agent_in_app(agentspace_app, agent_id)  # Changed from find_category_in_app
    
    agentspace_app["agents"] = [agt for agt in agentspace_app["agents"] if agt["id"] != agent_id]  # Changed from categories to agents
    return jsonify({"message": f"Agent with ID {agent_id} deleted successfully from app {agentspace_app['appName']}."})  # Changed from Category to Agent
